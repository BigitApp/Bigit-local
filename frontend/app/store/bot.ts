...
export const useBotStore = create<BotStore>()(
  persist(
    (set, get) => ({
      ...
      currentSession() {
        let currentBot = get().currentBot();
        if (!currentBot && Object.values(get().bots).length > 0) {
          const firstBot = Object.values(get().bots)[0];
          set((state): BotStore | Partial<BotStore> => {
            state.currentBotId = firstBot.id;
            return state;
          });
          currentBot = firstBot;
        }
